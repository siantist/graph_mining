...
class RandomWalk5(Kernel):
    ...
    def initialize(self):
        ...
        #error here: self.edge_list = edge_list
    ...
    def parse_input(self, X):
        ...
        if is_iter:
            for (idx, x) in enumerate(iter(X)): # only parse 0-th element (others are labels)
                ...
                weight_mat = weights(A, q, self.beta) # it's not a vector, it's a matrix
